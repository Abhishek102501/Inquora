...
import { FileStack, MessagesSquare, FileText, Layers, Upload, Plus, ArrowRight, Loader2 } from "lucide-react";
...
export default function DashboardPage() {
  const { documents, state: documentsState, error: documentsError } = useDocuments();
  const { conversations, state: conversationsState, error: conversationsError } = useConversations();

  const totalPages = documents.reduce((sum, d) => sum + (d.status === "ready" ? d.pages : 0), 0);
  const recentDocuments = documents.slice(0, 5);
  const recentConversations = conversations.slice(0, 5);
  const isLoading = documentsState === "loading" || conversationsState === "loading";
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-8 p-4 md:p-8">
      ...
      {isLoading ? (
        ...
        <>
          <StaggerGroup className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4" viewport={false} stagger={0.06}>
            <StaggerItem>
